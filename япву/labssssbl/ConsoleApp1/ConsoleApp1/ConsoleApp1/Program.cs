...
namespace ConsoleApp1
{
    class Program
    {
        ...
        {
            Console.Write($"Введите новое название журнала: ");
            string newNameJournal = Console.ReadLine();

            return newNameJournal;
        }
        ...
        private static void SelectingJournalToChangeName(Journal[] allJournals, Subscriber[] allSubscribers)
        {
            ...
            allJournals[number - 1].NameOfJournal = InputName();
        }
        public static double InputPrice()
        {
            double price;
            Console.Write($"Введите новую годовую стоимость подписки журнала: ");
            string input = Console.ReadLine();
            while (!Double.TryParse(input, out price) || price > 1000 || price < 1)
            {
                Console.Write($"Произошла ошибка. Введите новую годовую стоимость подписки журнала: ");
                input = Console.ReadLine();
            }
        ...
        private static void SelectingJournalToChangePriceSubscription(Journal[] allJournals, Subscriber[] allSubscribers)
        {
            ...
            allJournals[number - 1].SubscriptionPrice = InputPrice();
