...
{
    public class GazetterEntitySQLiteRepository
    {
        public static readonly string dataSource = @"D:\универ\4sem\япву\labssssbl\llllllrrrr67\Gazetter.db";
        public IEnumerable<Gazetter> GetAll()
        {
            using (GazetterContextSQLite db = new GazetterContextSQLite())
            {
                List<Gazetter> gazetters = new List<Gazetter>();
                var gazettersDB = db.Gazetters.ToList();
        ...
        {
            using (GazetterContextSQLite db = new GazetterContextSQLite(dataSource))
            {
